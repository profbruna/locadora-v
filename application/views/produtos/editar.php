            ...
            <div class="col-md-4">
                ...
                <select class="selectpicker" name="tipo_id" required="required">
                    <?php foreach ($tipos as $tipo) { ?>
                        <option <?php if ($tipo->id == $produto[0]->tipo_id) {
                        echo "selected='selected'";
                    } ?> value="<?php echo $tipo->id ?>" > <?php echo $tipo->descricao ?> </option>
<?php } ?>
                </select>
            </div>
            <div class="col-md-4">
                ...
                <select class="selectpicker" name="genero_id" required="required">
                    <?php foreach ($generos as $genero) { ?>
                        <option <?php if ($genero->id == $produto[0]->genero_id) {
                        echo "selected='selected'";
                    } ?> value="<?php echo $genero->id ?>" > <?php echo $genero->descricao ?> </option>
<?php } ?>
                </select>
            </div>
            <div class="col-md-4">
...
<?php foreach ($classificacoes as $classificacao) { ?>
                        <option <?php if ($classificacao->id == $produto[0]->classificacao_id) {
        echo "selected='selected'";
    } ?> value="<?php echo $classificacao->id ?>" > <?php echo $classificacao->descricao ?> </option>
<?php } ?>
                </select>
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="date" value="<?php echo $produto[0]->data_cadastro ?>" class="form-control" readonly="readonly" name="data_cadastro" value="<?php echo date("d-m-Y"); ?>" id="Cadastro" required=""/>
            </div>  
            <div class="col-md-6">
                <label for="detalhes">Detalhes</label>
                <textarea value="<?php echo $produto[0]->detalhes ?>"class="form-control" name="detalhes" id="Detalhes" required=""></textarea>            </div>
            <div class="col-md-6">
                <label for="dias_devolucao">Devolução</label>
                <input type="text" value="<?php echo $produto[0]->dias_devolucao ?>" class="form-control" name=" dias_devolucao" id="Devolucao" required=""/>
